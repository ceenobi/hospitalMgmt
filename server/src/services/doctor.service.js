...
const doctorService = {
  register: async (doctorData, next) => {
    const userExists = await Doctor.findOne({
      userId: doctorData.userId,
      role: "doctor",
    }).lean();
    if (userExists) {
      return next(errorResponse("Doctor already exists", 400));
    }
  ...
  getAllUserCollections: async (next) => {
  ...
  deleteDoctor: async (doctorId, next) => {
